#nohup poetry run accelerate launch train.py --config_path configs/train_conf2.json > log/train_conf2.log 2>&1 &
...
def main(
    ...
    loss_fct = CrossEntropyLoss(reduction="sum").cuda()
    ...
    def masked_loss(input_ids, attention_mask, logits, start_ids, end_ids):
        ...
        mask = torch.arange(seq_len, device=device).expand(batch_size, seq_len) >= start_ids.to(device).unsqueeze(1)
        ...
        mask[torch.arange(batch_size), (end_ids - 1).to(device)] = False
        mask = mask[:, :-1]
    ...
    def evaluate(test_loader):
        ...
        with torch.no_grad():
            for batch in test_loader:
                ...
                input_ids = input_ids[:, :max_ids].to("cuda")
                attention_mask = attention_mask[:, :max_ids].to("cuda")
    ...
    for epoch in range(num_epochs):
        ...
        model.save_pretrained(f'{output_dir}/checkpoint-{epoch}')
